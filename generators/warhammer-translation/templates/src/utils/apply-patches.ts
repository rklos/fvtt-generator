...
function patchTemplates(pkgName: string) {
  ...
  Object.entries(TEMPLATES_PATCHES).forEach(async ([ p, diffs ]) => {
    ...
    Object.values(diffs).forEach((diff) => {
      ...
      if (!patchedHtml) {
        <%= logFunctionName %>(`Failed to apply patch to ${originalPath}`);
        return;
      }
    ...
    <%= logFunctionName %>(`Overridden template: ${originalPath}`);
  });
}
...
function patchScripts(pkgName: string) {
  ...
  Object.entries(SCRIPTS_PATCHES).forEach(async ([ p, diffs ]) => {
    ...
    if (!script) {
      <%= logFunctionName %>(`Failed to find original script: ${path}`);
      return;
    }
    ...
    Object.values(diffs).forEach((diff) => {
      ...
      if (!patchedScript) {
        <%= logFunctionName %>(`Failed to apply patch to ${path}`);
        return;
      }
